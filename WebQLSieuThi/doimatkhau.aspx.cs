...
public partial class doimatkhau : System.Web.UI.Page
{
    ...
    {
        string sql;
        sql= "select * from TaiKhoan where TenND=" + int.Parse(Session["manvien"].ToString())+ " and MatKhau='"+MaHoaMatKhau(txtmkcu.Text.Trim())+"'";
        DataTable dt = kn.GetData(sql);
        if (dt.Rows.Count > 0)
        {
            try
            {
                sql = "update TaiKhoan set MatKhau='" + MaHoaMatKhau(txtmkmoi.Text.Trim()) + "' where TenND=" + int.Parse(Session["manvien"].ToString());
                kn.Execute(sql);
